...
_SCALAR_FIELDS = (
    'name', 'service_type', 'auth_type', 'upstream_hostname', 'dns_discovery_type', 'dns_lookup_family',
    'outlier_detection_enabled', 'outlier_detection_consecutive_5xx', 'outlier_detection_interval_seconds',
    'outlier_detection_base_ejection_time_seconds', 'outlier_detection_max_ejection_percent',
    'health_checks_enabled', 'health_check_timeout_seconds', 'health_check_interval_seconds',
    'health_check_unhealthy_threshold', 'health_check_healthy_threshold', 'healthy_panic_threshold',
)
...
class ServiceClusterTransformMixin_v1(BaseTransformMixin):
    ...
    def get_endpoint_operations(cls) -> Dict[str, EndpointOperation]:
        fields = ['name', 'service_type', 'auth_type', 'upstream_hostname', 'dns_discovery_type', 'dns_lookup_family',
                  'outlier_detection_enabled', 'outlier_detection_consecutive_5xx', 'outlier_detection_interval_seconds',
                  'outlier_detection_base_ejection_time_seconds', 'outlier_detection_max_ejection_percent',
                  'health_checks_enabled', 'health_check_timeout_seconds', 'health_check_interval_seconds',
                  'health_check_unhealthy_threshold', 'health_check_healthy_threshold', 'healthy_panic_threshold']
        return {
            'create': EndpointOperation(
                ...
                method='POST', fields=fields, required_for='create', order=1
            ),
            'update': EndpointOperation(
                path='/api/gateway/v1/service_clusters/{id}/',
                method='PATCH', fields=fields, path_params=['id'], required_for='update', order=1
            ),
            'delete': EndpointOperation(
                path='/api/gateway/v1/service_clusters/{id}/',
                method='DELETE', fields=[], path_params=['id'], required_for='delete', order=1
            ),
            'get': EndpointOperation(
                path='/api/gateway/v1/service_clusters/{id}/',
                method='GET', fields=[], path_params=['id'], required_for='find', order=1
            ),
            'list': EndpointOperation(
                path='/api/gateway/v1/service_clusters/',
                method='GET', fields=[], required_for='find', order=1
            ),
        }
