...
class ServiceTypeTransformMixin_v1(BaseTransformMixin):
    ...
    def from_ansible_data(cls, ansible_instance, context: Union[TransformContext, Dict[str, Any]]) -> 'APIServiceType_v1':
        ...
        ping_url = getattr(ansible_instance, 'ping_url', None)
        login_path = getattr(ansible_instance, 'login_path', None)
        logout_path = getattr(ansible_instance, 'logout_path', None)
        service_index_path = getattr(ansible_instance, 'service_index_path', None)
