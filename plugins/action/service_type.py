...
"""
Action plugin for ansible.platform.service_type module.
...
class ActionModule(BaseResourceActionPlugin):
    """
    Action plugin for service_type module.
    ...
    def run(self, tmp=None, task_vars=None):
        ...
        try:
            ...
            if not argspec:
                ...
                raise AnsibleError("Could not load DOCUMENTATION for service_type module")
            ...
            if operation == 'create' and validated_params.get('state') == 'present':
                try:
                    find_result = manager.execute(
                        ...
                        ansible_data={'name': st.name}
                    )
                    if find_result and find_result.get('id'):
                        operation = 'update'
                        st.id = find_result.get('id')
                except Exception:
                    pass

            # Delete: find by name to get id if not provided
            if operation == 'delete' and not st.id:
                try:
                    find_result = manager.execute(
                        operation='find',
                        module_name=self.MODULE_NAME,
                    ...
                    if find_result and find_result.get('id'):
                        ...
                        result.update({
                            ...
                            'msg': f"Service type '{st.name}' does not exist (already absent)"
                        })
                        return result
                except Exception:
                    result.update({
                ...
                if find_result and find_result.get('id'):
                    ...
                    for k in argspec_fields:
                        ...
                        if k in validated_params:
                            ...
                            merged[k] = find_result.get(k) or st.name
                        else:
                            merged[k] = None
                    for ro in read_only_fields:
                        if ro in find_result:
                            merged[ro] = find_result[ro]
                    merged.setdefault('name', st.name or find_result.get('name'))
                    st_data = {k: v for k, v in merged.items() if hasattr(AnsibleServiceType, k)}
                    st_data.setdefault('name', st.name)
                    st = AnsibleServiceType(**st_data)
                    operation = 'update'
                else:
                    operation = 'create'

            ansible_data = asdict(st)
            ...
            if self._task.check_mode and operation in ('create', 'update', 'delete'):
                if operation == 'create':
                    result.update({
                        ...
                        self.MODULE_NAME: {
                            'name': st.name,
                            'ping_url': getattr(st, 'ping_url', None),
                            'login_path': getattr(st, 'login_path', None),
                            'logout_path': getattr(st, 'logout_path', None),
                            'service_index_path': getattr(st, 'service_index_path', None),
                        },
                        'id': None,
                        'name': st.name,
                    })
                    ...
                    result.update({
                        'changed': bool(getattr(st, 'id', None)),
                        'failed': False,
                        self.MODULE_NAME: {'state': 'absent'},
                    })
                return result
            ...
            try:
                ...
                if operation == 'find' and ('not found' in str(e).lower() or 'resource with' in str(e).lower()):
                    result.update({
                        ...
                        self.MODULE_NAME: {},
                        'exists': False,
                        'msg': f"Service type '{st.name}' does not exist"
                    })
                    return result
                raise
            ...
            import traceback
            self._display.vvv(f"Error in service_type action plugin: {e}")
            result['failed'] = True
            result['msg'] = str(e)
