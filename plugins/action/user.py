...
class ActionModule(BaseResourceActionPlugin):
    ...
    def run(self, tmp: object = None, task_vars: Optional[dict] = None) -> dict:
        ...
        try:
            ...
            user_data = {k: v for k, v in validated_params.items() if v is not None and k not in auth_params}
            ...
            if operation == "create" and validated_params.get("state") == "present":
                try:
                    ...
                    find_result = manager.execute(operation="find", module_name=self.MODULE_NAME, ansible_data=find_data)
            ...
            if operation == "enforced":
                ...
                try:
                    find_result = manager.execute(operation="find", module_name=self.MODULE_NAME, ansible_data={"username": user.username})
                except ValueError:
                    find_result = None
                if find_result and find_result.get("id"):
            ...
            try:
                ...
                if operation == "find" and ("not found" in str(e).lower() or "resource with" in str(e).lower()):
                    result.update({"changed": False, "failed": False, self.MODULE_NAME: {}, "exists": False, "msg": f"User '{user.username}' does not exist"})
                    return result
                raise
            ...
            filtered_result = {k: v for k, v in manager_result.items() if k in argspec_resource_fields and k not in _internal_keys}
            try:
                validated_output = self._validate_data(
                    {k: v for k, v in filtered_result.items() if k in argspec_fields and k not in _ansible_directives}, argspec, "output"
                )
                ...
                validated_output = {k: v for k, v in manager_result.items() if k not in _excluded}
