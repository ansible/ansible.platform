...
"""
Action plugin for ansible.platform.authenticator module.
...
class ActionModule(BaseResourceActionPlugin):
    ...
    def run(self, tmp=None, task_vars=None):
        ...
        try:
            ...
            if not argspec:
                raise AnsibleError("Could not load DOCUMENTATION for authenticator module")
                ...
                try:
                    ...
                    if find_result and find_result.get('id'):
                        ...
                        result.update({
                            ...
                            'msg': "Authenticator '%s' does not exist (already absent)" % auth.name
                        })
                        return result
                except Exception:
                    result.update({
            ...
            if operation == 'enforced':
                ...
                if find_result and find_result.get('id'):
                    ...
                    merged.setdefault('name', auth.name or find_result.get('name'))
                    auth_data = {k: v for k, v in merged.items() if hasattr(AnsibleAuthenticator, k)}
                    auth = AnsibleAuthenticator(**auth_data)
                    operation = 'update'
                else:
                    operation = 'create'
            ...
            try:
                ...
                if operation == 'find' and ('not found' in str(e).lower() or 'resource with' in str(e).lower()):
                    result.update({
                        ...
                        'exists': False, 'msg': "Authenticator '%s' does not exist" % auth.name
                    })
                    return result
                raise
            read_only_fields = {'id', 'created', 'modified', 'url'}
            ...
            import traceback
            self._display.vvv("Error in authenticator action plugin: %s" % e)
            result['failed'] = True
            result['msg'] = str(e)
