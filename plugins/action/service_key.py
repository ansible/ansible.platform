...
"""
Action plugin for ansible.platform.service_key module.
...
class ActionModule(BaseResourceActionPlugin):
    ...
    def run(self, tmp=None, task_vars=None):
        ...
        try:
            ...
            if not argspec:
                raise AnsibleError("Could not load DOCUMENTATION for service_key module")
                ...
                try:
                    ...
                    if find_result and find_result.get('id'):
                        ...
                        result.update({
                            ...
                            'msg': "Service key '%s' does not exist (already absent)" % sk.name
                        })
                        return result
                except Exception:
                    result.update({
            ...
            if operation == 'enforced':
                ...
                if find_result and find_result.get('id'):
                    ...
                    merged.setdefault('name', sk.name or find_result.get('name'))
                    sk_data = {k: v for k, v in merged.items() if hasattr(AnsibleServiceKey, k)}
                    sk = AnsibleServiceKey(**sk_data)
                    operation = 'update'
                else:
                    operation = 'create'
            ...
            try:
                ...
                if operation == 'find' and ('not found' in str(e).lower() or 'resource with' in str(e).lower()):
                    result.update({
                        ...
                        'exists': False, 'msg': "Service key '%s' does not exist" % sk.name
                    })
                    return result
                raise
            read_only_fields = {'id', 'created', 'modified', 'url'}
            ...
            import traceback
            self._display.vvv("Error in service_key action plugin: %s" % e)
            result['failed'] = True
            result['msg'] = str(e)
