...
class ActionModule(BaseResourceActionPlugin):
    """Action plugin for service module."""
    ...
    def run(self, tmp=None, task_vars=None):
        ...
        try:
            ...
            if not argspec:
                ...
                raise AnsibleError("Could not load DOCUMENTATION for service module")
            ...
            resource = AnsibleService(**resource_data)
                ...
                try:
                    ...
                    if find_result and find_result.get('id'):
                        ...
                        result.update({
                            ...
                            'msg': "Service '%s' does not exist (already absent)" % resource.name,
                        })
                        return result
                except Exception:
                    result.update({
            ...
            try:
                ...
                if operation == 'find' and ('not found' in str(e).lower() or 'resource with' in str(e).lower()):
                    result.update({'changed': False, 'failed': False,
                                   ...
                                   'msg': "Service '%s' does not exist" % resource.name})
                    return result
                raise
            ...
            import traceback
            self._display.vvv("Error in service action plugin: %s" % e)
            result['failed'] = True
            result['msg'] = str(e)
