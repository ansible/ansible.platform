...
"""
Action plugin for ansible.platform.ui_plugin_route module.
...
class ActionModule(BaseResourceActionPlugin):
    """Action plugin for ui_plugin_route module."""
    ...
    def run(self, tmp=None, task_vars=None):
        ...
        try:
            ...
            if not argspec:
                ...
                raise AnsibleError("Could not load DOCUMENTATION for ui_plugin_route module")
            ...
            resource = AnsibleUIPluginRoute(**resource_data)
                ...
                try:
                    ...
                    if find_result and find_result.get('id'):
                        ...
                        result.update({
                            ...
                            'msg': "UIPluginRoute '%s' does not exist (already absent)" % resource.name,
                        })
                        return result
                except Exception:
                    result.update({
            ...
            try:
                ...
                if operation == 'find' and ('not found' in str(e).lower() or 'resource with' in str(e).lower()):
                    result.update({'changed': False, 'failed': False,
                                   ...
                                   'msg': "UIPluginRoute '%s' does not exist" % resource.name})
                    return result
                raise
            ...
            import traceback
            self._display.vvv("Error in ui_plugin_route action plugin: %s" % e)
            result['failed'] = True
            result['msg'] = str(e)
