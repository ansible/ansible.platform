...
"""
Action plugin for ansible.platform.service_cluster module.
...
class ActionModule(BaseResourceActionPlugin):
    ...
    def run(self, tmp=None, task_vars=None):
        ...
        try:
            ...
            if not argspec:
                raise AnsibleError("Could not load DOCUMENTATION for service_cluster module")
                ...
                try:
                    ...
                    if find_result and find_result.get('id'):
                        ...
                        result.update({
                            ...
                            'msg': "Service cluster '%s' does not exist (already absent)" % sc.name
                        })
                        return result
                except Exception:
                    result.update({
            ...
            if operation == 'enforced':
                ...
                if find_result and find_result.get('id'):
                    ...
                    merged.setdefault('name', sc.name or find_result.get('name'))
                    sc_data = {k: v for k, v in merged.items() if hasattr(AnsibleServiceCluster, k)}
                    sc = AnsibleServiceCluster(**sc_data)
                    operation = 'update'
                else:
                    operation = 'create'
            ...
            try:
                ...
                if operation == 'find' and ('not found' in str(e).lower() or 'resource with' in str(e).lower()):
                    result.update({
                        ...
                        'exists': False, 'msg': "Service cluster '%s' does not exist" % sc.name
                    })
                    return result
                raise
            read_only_fields = {'id', 'created', 'modified', 'url'}
            ...
            import traceback
            self._display.vvv("Error in service_cluster action plugin: %s" % e)
            result['failed'] = True
            result['msg'] = str(e)
