...
options:
    config:
      description:
        - A list of service resource configurations.
        - Each entry represents a desired service state.
      type: list
      elements: dict
      suboptions:
        name:
...
EXAMPLES = """
- name: Create service resources (merged)
  ansible.platform.services:
    config:
      - name: "my-service"
    state: merged
...
- name: Delete specific service resources
  ansible.platform.services:
    config:
      - name: "my-service"
    state: deleted

- name: Override — ensure only these service resources exist
  ansible.platform.services:
    config:
...
RETURN = """
before:
  description: The state of the service resources before this run.
  returned: when state is not gathered
  type: list
  elements: dict
after:
  description: The state of the service resources after this run.
  returned: when state is not gathered
  type: list
  elements: dict
gathered:
  description: The gathered service resource state (read-only).
