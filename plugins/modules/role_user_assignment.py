...
def main():
    ...
    module = AAPModule(
        ...
        mutually_exclusive=[("user", "user_ansible_id"), ("object_ids", "object_ansible_id"), ("object_ids", "object_id"), ("object_id", "object_ansible_id")],
    )
    ...
    entity_type = next((mapped for prefix, mapped in role_map.items() if role_definition_str.startswith(prefix)), None)
    ...
    if role_definition_str.lower().startswith("platform") and role_definition["id"] == 1:
        ...
        for entity in object_param:
            if not isinstance(entity, int):
                ...
                if response is None:
                    module.fail_json(msg=f"Unable to find {entity_type} with name or id: {entity}")
