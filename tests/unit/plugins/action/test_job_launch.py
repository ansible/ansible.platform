...
"""Unit tests for the job_launch action plugin (check_mode, wait-timeout handling)."""
...
module: job_launch
options:
  name: {type: str, required: true, aliases: [job_template]}
...
class TestWaitTimeoutHandling(unittest.TestCase):
    def test_timeout_preserves_id_and_status(self):
        ...
        mock_manager.execute.side_effect = WaitTimeoutError(
            "Timed out waiting for job_launch 9 to complete after 5 seconds (status: pending)",
