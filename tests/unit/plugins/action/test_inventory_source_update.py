...
"""Unit tests for the inventory_source_update action plugin (check_mode, wait-timeout handling)."""
...
from ansible_collections.ansible.platform.plugins.action.inventory_source_update import ActionModule
...
options:
  name: {type: str, required: true, aliases: [inventory_source]}
...
class TestWaitTimeoutHandling(unittest.TestCase):
    def test_timeout_preserves_id_and_status(self):
        ...
        mock_manager.execute.side_effect = WaitTimeoutError(
            "Timed out waiting for inventory_source_update 9 to complete after 5 seconds (status: pending)",
