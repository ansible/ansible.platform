...
1. [What a Resource Module Is](#section-1-what-a-resource-module-is)
2. [The Five States](#section-2-the-five-states)
...
The module's job is **convergence**: you declare the desired state of a thing, and the
module figures out what operations are needed to make reality match. The user never
thinks about HTTP verbs, endpoint paths, or payload structure. They think about the
entity and the state they want it in.

### Core Properties

Every `ansible.platform` resource module exhibits these properties:
...
The defining behavior of a resource module is **convergence**. Given a desired state declaration, the module:
...
Used by modules whose fields are collections (e.g. role assignments, permission sets).

```yaml
- name: Add a user to organizations (without replacing existing assignments)
  ansible.platform.user:
    username: alice
    organizations:
      - engineering
...
- name: Assign roles to a user (cumulative, not replacive)
  ansible.platform.user_role:
    username: alice
    roles:
      - admin
    state: merged
...
**Formal definition**: For list/dict fields, `merged` performs union instead of replacement.
Non-collection fields behave like `present`. If `E` is ∅, create with `D`. If `E` is not ∅
and has collections, merge `D` collections into `E` collections (no removal).

**Edge cases**:
- For scalar fields, behaves identically to `present`
- For collections, removes nothing — only adds/updates
- Used when you want cumulative assignment (add to a group, not replace)
- Idempotent: idempotent when the desired items are already in the collection
...
- **Full state semantics** — `present`, `absent`, `exists`, `enforced`, `merged`
...
API calls. This is guaranteed for all resource modules.
...
A module that cannot do this loop is not a resource module. It is an API client with
YAML syntax. The convergence contract is what separates infrastructure automation from
...
the versioned data model, makes this possible.

### The Problem: Version Divergence

Gateway API versions introduce field changes, endpoint changes, and behavior changes:

- AAP 2.4: Basic user management, limited organization features
- AAP 2.5: User groups, enhanced organization hierarchy, service clusters
- AAP 3.x: Role-based access control overhaul, new permission model

A naive collection (one module per version) requires 3× the code and maintenance.
...
This identical task works on AAP 2.4, 2.5, and 3.x because:
...
2. **Version-specific mixins** are loaded dynamically:
   - AAP 2.4 task → `UserTransformMixin_v24` (legacy API)
   - AAP 2.5 task → `UserTransformMixin_v25` (user groups support)
   - AAP 3.x task → `UserTransformMixin_v3x` (RBAC model)

3. **Field mapping** is version-aware:
   - v2.4: `organizations` → `organization_ids` (direct IDs)
   - v2.5: `organizations` → `org_names` + name lookup
   - v3.x: `organizations` → role assignments (different model entirely)

### Practical Example: Multi-Version Playbook

```yaml
---
- name: Ensure users exist across AAP 2.4, 2.5, and 3.x
  hosts: aap_gateways
  tasks:
...
## Summary: Endpoint vs. Resource Model

| Aspect | Endpoint Model | Resource Model |
|--------|----------------|----------------|
| **Module count** | ~200 (1 per endpoint) | ~20 (1 per entity) |
| **Tasks to configure user with orgs** | 2-3 | 1 |
| **Compliance enforcement** | Not possible | Built-in (`state: enforced`) |
| **Compliance audits** | Manual scripting | Native (check mode + enforced) |
| **API rate limit risk** | High | Low (only changes need API calls) |
...
The resource module pattern turns Ansible playbooks from **imperative API scripts**
into **declarative infrastructure definitions**. Instead of "call this endpoint, then
that one, then the other," the user writes "this user should exist in these organizations,
this port should be configured this way, only these feature flags should be enabled."
